...
import { Activity, CalendarDays, FlaskConical, TrendingUp, Users } from 'lucide-react';
            ...
            <div className="space-y-6 p-6">
                ...
                <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
                    {stats.map((stat) => (
                        <div
                            key={stat.label}
                            className="group relative overflow-hidden rounded-2xl border border-ocean-aqua/30 bg-white p-5 shadow-sm transition-all duration-300 hover:shadow-lg hover:shadow-ocean-teal/5 hover:-translate-y-0.5 dark:bg-card dark:border-sidebar-border"
                        >
                            {/* Gradient accent bar */}
                            <div
                                className={cn(
                            ...
                            <div className="mt-3 flex items-center gap-1.5">
                                <TrendingUp
                                    className={cn(
                                        'h-3.5 w-3.5',
                                        stat.trend === 'up' ? 'text-emerald-500' : 'text-ocean-coral',
                                    )}
                                />
                                <span
                                    className={cn(
                                        'text-xs font-medium',
                                        stat.trend === 'up' ? 'text-emerald-600' : 'text-ocean-coral',
                                    )}
                                >
                                    {stat.change}
                                </span>
                ...
                <div className="grid gap-5 lg:grid-cols-2">
                    ...
                    <div className="rounded-2xl border border-ocean-aqua/30 bg-white p-6 shadow-sm transition-all duration-300 hover:shadow-md dark:bg-card dark:border-sidebar-border">
                        ...
                        <div className="mt-6 grid grid-cols-7 gap-2">
                            {['Lun', 'Mar', 'Mer', 'Jeu', 'Ven', 'Sam', 'Dim'].map((day, i) => (
                                <div key={day} className="flex flex-col items-center gap-2">
                                    <span className="text-xs text-slate-400 dark:text-sidebar-foreground/50">{day}</span>
                                    <div
                                        className={cn(
                                            ...
                                            i === 6 ? 'h-8' : `h-${[12, 16, 10, 20, 14, 6, 8][i]}`,
                                        )}
                                        style={{ height: `${[40, 55, 35, 70, 50, 20, 28][i]}%` }}
                                    />
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="rounded-2xl border border-ocean-aqua/30 bg-white p-6 shadow-sm transition-all duration-300 hover:shadow-md dark:bg-card dark:border-sidebar-border">
                        ...
                        <div className="mt-6 space-y-4">
                            {[
                                { name: 'Sophie Martin', time: '09:30', status: 'Consultation' },
                                { name: 'Ahmed Benali', time: '10:15', status: 'Suivi' },
                                { name: 'Marie Dubois', time: '11:00', status: 'Urgence', critical: true },
                                { name: 'Paul Lefèvre', time: '14:30', status: 'Bilan' },
                                { name: 'Lucie Petit', time: '15:45', status: 'Consultation' },
                            ].map((patient, i) => (
                                <div
                                    key={i}
                                    className="flex items-center gap-3 rounded-xl border border-ocean-aqua/20 bg-ocean-sand/50 p-3 transition-colors hover:bg-ocean-aqua/20 dark:bg-sidebar-accent dark:border-sidebar-border"
                                >
                                    <div className={cn(
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <p className="text-sm font-medium text-ocean-deep dark:text-sidebar-foreground truncate">
                                            {patient.name}
                                        </p>
